...
$(() => {
  ...
  $(".qrtrFltr").click(function () {
    ...
    fetch(`https://qa-heal.planx-pla.net/dashboard/Secure/QA/${year}/${quarter}/`).then(
      (r) => {
        if (r.status === 401) {
          window.location.replace("https://qa-heal.planx-pla.net/login/?next=/dashboard/Secure/qa-reports/index.html");
        }
        return r.json()
      }
    ).then(
